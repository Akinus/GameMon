...
// Author: Akinus21                                                    #
// -----                                                               #
// Last Modified: Sat, 05 Nov 2022 @ 14:57:08                          #
...
use std::{process::{Command}, os::windows::{process::CommandExt}, io::Write, fs::{OpenOptions, File}, path::{Path}, cmp::Ordering};
    ...
    ($a:expr) => {
        {
            ...
            write_key("defaults", "exit-reason", "Shutdown", &mut i);
            std::process::exit($a);
        }
    };
    ...
    ($b:expr) => {
        {
            ...
            let path = ini_file();
            let mut i = Ini::load_from_file(path).unwrap();
            write_key("defaults", "exit-reason", $b, &mut i);
            std::process::abort();           
        }
    };
    ...
    fn events(&self) {
        self.wnd.on().wm_create({ // happens once, right after the window is created
			...
			move |_| {
                let path = ini_file();
                let mut i = Ini::load_from_file(path).unwrap();
                let defaults = get_defaults(&mut i);
        ...
        self.btn_save.on().bn_clicked({
            ...
            move || {
                let path = ini_file();
                let mut i = Ini::load_from_file(path).unwrap();
                write_key(&"defaults", "openRGBPath", self2.edit_openRGBpath.text().as_str(), &mut i);
                write_key(&"defaults", "voiceAttackPath", self2.edit_voiceAttackPath.text().as_str(), &mut i);
                write_key(&"defaults", "defaultORGBProfile", self2.edit_defaultORGBProfile.text().as_str(), &mut i);
                write_key(&"defaults", "defaultSRGBProfile", self2.edit_defaultSRGBProfile.text().as_str(), &mut i);
                write_key(&"defaults", "screensaver_orgb_profile", self2.edit_screensaver_orgb_profile.text().as_str(), &mut i);
                write_key(&"defaults", "screensaver_srgb_profile", self2.edit_screensaver_srgb_profile.text().as_str(), &mut i);
                write_key(&"defaults", "night_hour_orgb_profile", self2.edit_night_hour_orgb_profile.text().as_str(), &mut i);
                write_key(&"defaults", "night_hour_srgb_profile", self2.edit_night_hour_srgb_profile.text().as_str(), &mut i);
                write_key(&"defaults", "orgb_port", self2.edit_orgb_port.text().as_str(), &mut i);
                write_key(&"defaults", "orgb_address", self2.edit_orgb_address.text().as_str(), &mut i);
    ...
    fn events(&self) {
        self.wnd.on().wm_create({ // happens once, right after the window is created
			...
			move |_| {
                ...
                match &sec.as_str() {
                    &"defaults" => (),
                    _ => {
                        let section = get_section_with_sec(&sec);
        ...
        self.main_list.on().lbn_sel_change({ 
			...
			move || {
                ...
                let prop = i.section(Some(sec)).unwrap();
                match &sec.as_str() {
                    &"defaults" => (),
                    &"Idle" => {
                        let section = get_section(prop);
        ...
        self.radio_game_win.on().bn_clicked({
            ...
            move || {
                ...
                if let Some(game_or_win) = self2.radio_game_win.checked() {
                    ...
                    let path = ini_file();
                    let mut i = Ini::load_from_file(path).unwrap();
                    write_key(&sec, "game-or-win", &gow, &mut i);
                    msg_box("GameMon", "Saved!");
                }
                Ok(())
            }
            ...
            move || {
                ...
                match &section.other_commands.as_str() {
                    &"" => {
                        ...
                        let path = ini_file();
                        let mut i = Ini::load_from_file(path).unwrap();
                        write_key(&sec, "other_commands", &final_string, &mut i);
                    },
                    s => {
                        let cmds = s.split(" && ");
                        for c in cmds {
        ...
        self.btn_save.on().bn_clicked({
            ...
            move || {
                ...
                let path = ini_file();
                let mut i = Ini::load_from_file(path).unwrap();
                write_key(&sec, "exeName", &self2.edit_exe.text(), &mut i);
                write_key(&sec, "gameWindowName", &self2.edit_win_name.text(), &mut i);
                ...
                write_key(&sec, "nameOfahk", &self2.edit_ahk_name.text(), &mut i);
                write_key(&sec, "pathToahk", &self2.edit_ahk_path.text(), &mut i);
                write_key(&sec, "OpenRGBprofile", &self2.edit_orgb_profile.text(), &mut i);
                write_key(&sec, "voiceAttackProfile", &self2.edit_va_profile.text(), &mut i);
                write_key(&sec, "SignalRGBprofile", &self2.edit_srgb_profile.text(), &mut i);
                if let Some(game_or_win) = self2.radio_game_win.checked() {
                    let gow = game_or_win.hwnd().GetWindowText()?;
                    write_key(&sec, "game-or-win", &gow, &mut i);
                }
                ...
                write_key(&sec, "orgb_port", &self2.edit_orgb_port.text(), &mut i);
                write_key(&sec, "orgb_address", &self2.edit_orgb_address.text(), &mut i);
                msg_box("GameMon", "Saved!");
                Ok(())
            }
        });
...
fn reset_running() -> String{
    ...
    for (sec, _prop) in i.iter(){
        if &sec.unwrap() != &"General" {
            write_key(&sec.unwrap().to_string(), "running", "False", &mut i2);
        };
    };
    write_key("defaults", "gameon", "False", &mut i);
    write_key("General", "running", "True", &mut i);
    write_key("General", "running_pid", "0", &mut i);
...
fn reg_check(){
    ...
    match disp {
        REG_CREATED_NEW_KEY => {
            log!(&"A new key has been created".to_string());
            let ini_file: String = std::env::current_dir().unwrap().to_str().unwrap().to_owned();
            match reg_write_value("InstallDir".to_string(), format!("{}", &ini_file)) {
                Ok(_) => {
            ...
            match reg_write_value("display".to_string(), (&"on").to_string()) {
                Ok(_) => {
...
fn reg_write_value(name: String, value: String) -> Result<(), std::io::Error>{
    let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
    let path = Path::new("Software").join("GameMon");
...
fn initialize_ini(){
    ...
    let inifile = ini_file();
    let mut i = Ini::load_from_file(&inifile).unwrap();
    let e = std::path::Path::new(&inifile).exists();
    
    match e {
        false => {
            let path = Path::new(&inifile);
            File::create(&path).unwrap();
            
            let section_name = "General".to_string();
            write_key(&section_name, "exeName", "", &mut i);
            write_key(&section_name, "gameWindowName", "", &mut i);
            write_key(&section_name, "nameOfahk", "", &mut i);
            write_key(&section_name, "pathToahk", "", &mut i);
            write_key(&section_name, "OpenRGBprofile", "General", &mut i);
            write_key(&section_name, "SignalRGBprofile", "General", &mut i);
            write_key(&section_name, "voiceAttackProfile", "", &mut i);
            write_key(&section_name, "game-or-win", "Game", &mut i);
            write_key(&section_name, "running_pid", "", &mut i);
            write_key(&section_name, "running", "", &mut i);

            let section_name = "Idle".to_string();
            write_key(&section_name, "exeName", "300", &mut i);
            write_key(&section_name, "gameWindowName", "2100-0600", &mut i);
            write_key(&section_name, "nameOfahk", "", &mut i);
            write_key(&section_name, "pathToahk", "", &mut i);
            write_key(&section_name, "OpenRGBprofile", "", &mut i);
            write_key(&section_name, "SignalRGBprofile", "", &mut i);
            write_key(&section_name, "voiceAttackProfile", "", &mut i);
            write_key(&section_name, "game-or-win", "Game", &mut i);
            write_key(&section_name, "running_pid", "", &mut i);
            write_key(&section_name, "running", "", &mut i);
            ...
            write_key(&section_name, "exit-reason", "", &mut i);
            write_key(&section_name, "pathToSchemas", "", &mut i);
            write_key(&section_name, "orgb_port", "6742", &mut i);
            write_key(&section_name, "orgb_address", "127.0.0.1", &mut i);
            write_key(&section_name, "gameon", "False", &mut i);
            write_key(&section_name, "running", "", &mut i);
            write_key(&section_name, "window_flag", "General", &mut i);
            write_key(&section_name, "screensaver_orgb_profile", "General", &mut i);
            write_key(&section_name, "screensaver_srgb_profile", "Screen Ambience", &mut i);
...
fn ahk_pid(sec: &String, ini: &Ini) -> Result<u32, String> {
    let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
    let autohotkey = hklm.open_subkey("SOFTWARE\\AutoHotkey").unwrap();
    let version: String = autohotkey.get_value("Version").unwrap();
    
    let prop = ini.section(Some(sec)).unwrap();
    let ahk = prop.get("pathToahk").unwrap().to_string();
...
fn get_section(prop: &Properties) -> Instance {
    return Instance {
        exe_name: prop.get("exeName").unwrap().to_string(),
        game_window_name: prop.get("gameWindowName").unwrap().to_string(),
        name_ofahk: prop.get("nameOfahk").unwrap().to_string(),
        path_toahk: prop.get("pathToahk").unwrap().to_string(),
        open_rgbprofile: prop.get("OpenRGBprofile").unwrap().to_string(),
        signal_rgbprofile: prop.get("SignalRGBprofile").unwrap().to_string(),
        voice_attack_profile: prop.get("voiceAttackProfile").unwrap().to_string(),
        game_or_win: prop.get("game-or-win").unwrap().to_string(),
        running: prop.get("running").unwrap().to_string(),
        running_pid: prop.get("running_pid").unwrap().to_string(),
        other_commands: prop.get("other_commands").unwrap().to_string()
    };
}

fn get_section_with_sec(sec: &String) -> Instance {
    let path = ini_file();
    let i = Ini::load_from_file(path).unwrap();
    let prop = i.section(Some(sec)).unwrap();
...
fn get_defaults(ini: &mut Ini) -> Defaults {
    let prop = ini.section(Some("defaults")).unwrap();
    return Defaults {
        openrgb_path: prop.get("openRGBpath").unwrap().to_string(),
        exit_reason: prop.get("exit-reason").unwrap().to_string(),
        voice_attack_path: prop.get("voiceAttackPath").unwrap().to_string(),
        default_orgb_profile: prop.get("defaultORGBProfile").unwrap().to_string(),
        default_srgb_profile: prop.get("defaultSRGBProfile").unwrap().to_string(),
        screensaver_orgb_profile: prop.get("screensaver_orgb_profile").unwrap().to_string(),
        screensaver_srgb_profile: prop.get("screensaver_srgb_profile").unwrap().to_string(),
        night_hour_orgb_profile: prop.get("night_hour_orgb_profile").unwrap().to_string(),
        night_hour_srgb_profile: prop.get("night_hour_srgb_profile").unwrap().to_string(),
        orgb_port: prop.get("orgb_port").unwrap().to_string(),
        orgb_address: prop.get("orgb_address").unwrap().to_string(),
        gameon: prop.get("gameon").unwrap().to_string(),
        window_flag: prop.get("window_flag").unwrap().to_string(),

    };
}

fn write_key(sec_name: &str, key_name: &'static str, key_value: &str, ini: &mut Ini){
    let i = ini;
...
fn write_section(sec_name: &String, ini: &mut Ini){
    let mut i = ini;
    write_key(&sec_name, "exeName", "", &mut i);
    write_key(&sec_name, "gameWindowName", "", &mut i);
    write_key(&sec_name, "nameOfahk", "", &mut i);
    write_key(&sec_name, "pathToahk", "", &mut i);
    write_key(&sec_name, "OpenRGBprofile", "", &mut i);
    write_key(&sec_name, "voiceAttackProfile", "", &mut i);
    write_key(&sec_name, "SignalRGBprofile", "", &mut i);
    write_key(&sec_name, "game-or-win", "", &mut i);
    write_key(&sec_name, "running", "", &mut i);
    write_key(&sec_name, "running_pid", "", &mut i);
}
...
fn test(){
    ...
    // log!(format!("TEST TEST TEST {test}"), "d");
    ...
    //             hm.insert(k, v);
    //         }
    //     d_log(&hm["exeName"].to_string());
    
    // };
    // exit_app(Some(0), Some("Test"));
}
...
async fn main() {
    ...
    loop {
        ...
        for (sect, prop) in io.iter(){
                        ...
                        Ordering::Greater => { // PAST IDLE TIME!!!!!!
                            match &section.running.as_str() {
                                &"False" => { //IDLE IS NOT RUNNING
                                    ...
                                    write_key("defaults", "gameon", "True", &mut i);
                                    write_key(&sec, "running", "True", &mut i);
                                    write_key("General", "running", "False", &mut i);
                        ...
                        _ => { // NOT PAST IDLE TIME
                            match &section.running.as_str() {
                                &"True" => { //IDLE IS RUNNING
                                    ...
                                    //change values
                                    write_key("General", "running", "True", &mut i);
                                    write_key(&sec, "running", "False", &mut i);
                                    write_key("defaults", "gameon", "False", &mut i);

                                    //run extra commands
                                    let ahk_pid = ahk_pid(&sec.to_string(), &mut i);
                                ...
                                Ok(msg) => { //Program Found
                                    match &section.running.as_str() {
                                        &"False" => { // Profile Off
                                            ...
                                            //change values
                                            write_key(&sec, "running", "True", &mut i);
                                            write_key(&sec, "running_pid", &msg.to_string(), &mut i);
                                            write_key("General", "running", "False", &mut i);
                                            write_key("defaults", "gameon", "True", &mut i);
                            ...
                            if &active_win == &section.exe_name{ // ** WINDOW IS ACTIVE **
                                match &section.running.as_str() {
                                    &"False" => { //Profile Off
                                        ...
                                        //change values
                                        write_key(&sec, "running", "True", &mut i);
                                        write_key(&sec, "running_pid", &active_pid.to_string(), &mut i);
                                        write_key("General", "running", "False", &mut i);
                                        write_key("defaults", "window_flag", &sec, &mut i);
                                ...
                                match &section.running.as_str() {
                                    &"True" => { // Profile On
                                        ...
                                        write_key(&sec, "running", "False", &mut i);
                                        if &win_flag == &sec {
                                            write_key("defaults", "window_flag", "General", &mut i);
                                            write_key("General", "running", "True", &mut i);
            ...
            let general = get_section(i.section(Some("General")).unwrap());
            ...
            if &general.running == "True" {
                if &general.running_pid == "0" {
                    ...
                    write_key("General", "running_pid", "1", &mut i);
                } 
            } else {
                match general.running_pid.as_str() {
                    "0" => (),
                    _ => write_key("General", "running_pid", "0", &mut i),
                };
            };
